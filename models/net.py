...
class Encoder(nn.Module):
    def __init__(self, inp_channels=1, out_channels=1, dim=64, num_blocks=[3, 3], heads=[8, 8, 8], ffn_expansion_factor=2, bias=False, LayerNorm_type='WithBias'):
    ...
    def forward(self, input_img):
        inp_emb = self.patch_embed(input_img)
        encoder_out = self.encoder_level1(inp_emb)
        detail_feature = self.detailFeature(encoder_out)
        return detail_feature, encoder_out
...
class Decoder(nn.Module):
    ...
    def forward(self, input_img, all_feature):
        output = self.encoder_level2(all_feature)
        if input_img is not None:
            output = self.output(output) + input_img
        else:
...
class INN(nn.Module):
    def __init__(self, num_layers=3):
        ...
        INNmodules = [INNNode() for _ in range(num_layers)]
...
class INNNode(nn.Module):
    def __init__(self):
        super(INNNode, self).__init__()
...
def iv_fuse_sche(sche='mean', f_v=None, f_i=None):
    if sche == 'max':
        x = torch.max(f_i, f_v)
    elif sche == 'sum':
        x = f_i + f_v
